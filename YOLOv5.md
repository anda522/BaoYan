...
二阶段的核心思想是首先提出proposal框，通过第一阶段的网络回归出目标框的大概位置、大小及是前景的概率，第二阶段是通过另一个网络回归出目标框的位置、大小及类别；二阶段的算法R-CNN（区域卷积神经网络）

而一阶段网络的核心是，对于输入图像，通过网络直接回归出目标大小、位置和类别。一阶段的算法YOLO，SSD
